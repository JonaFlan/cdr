...
//
// ADAPTED FROM 
// http://a.teall.info/teal.js


console.log("corriendose en los dados...");
...
// 
// ADAPTED FROM
// http://a.teall.info/dice/dice.js
//
...
(function(dice) {
    ...
    this.dice_box.prototype.reinit = function(container, dimentions) {
        ...
        this.w = this.cw * Math.min(500/this.cw,500/this.ch);
        this.h = this.ch * Math.min(500/this.cw,500/this.ch);
        this.w = this.cw * Math.min(500/this.cw,500/this.ch);
        this.h = this.ch * Math.min(500/this.cw,500/this.ch);
    ...
    this.dice_box.prototype.check_if_throw_finished = function() {
        var res = true;
        var e = 6;
        if (this.iteration < 5 / that.frame_rate) {
    ...
    function get_dice_value(dice) {
        ...
        var closest_face = null, closest_angle = Math.PI * 2;
        ...
        if (closest_face !== null) {
        	matindex = closest_face.materialIndex - 1;
...
// 
// ADAPTED FROM
// http://a.teall.info/dice/main.js
//

function dice_initialize(container) {
    var canvas = $t.id('canvas');
    var label = $t.id('resultText');
    var set = $t.id('rollConfig');
    var logsHtml = $t.id('rollLog');
	var logs = JSON.parse (localStorage.getItem('Bitstream630_qwEeLK_log')) || [];
    ...
    function after_roll(notation, result) {
    	var res = format_result(notation, result);
        label.innerHTML = res;
    	logs.push({time:Date.now(), notation:set.value, result:res});
		localStorage.setItem('Bitstream630_qwEeLK_log', JSON.stringify(logs));
    	refresh_log();
    }
    
    function format_result(notation, result) {
        var res = result.join(' ');
    ...
    function refresh_log() {
      logsHtml.innerHTML = logs.reduceRight(function (a, c) {
          ...
          return a + `<p class="small mb-1"><strong>${dateString} ${timeString}</strong><br>${c.notation} = ${c.result}</p>`;
      }, '');
  }
  
	
	$t.bind($t.id('clearLog'), ['mouseup', 'touchend'], function(ev) {
        ev.stopPropagation();
        logs = [];
        refresh_log();
	});
	$t.bind($t.id('toggleSave'), ['mouseup', 'touchend'], function(ev) {
		ev.stopPropagation();
        $t.id('rollName').value = '';
        var n = $t.id('saveBlock');
        n.style.display = (n.style.display == 'none') ? 'block' : 'none';
	});
	$t.bind($t.id('saveThrow'), ['mouseup', 'touchend'], function(ev) {
		ev.stopPropagation();
		var name = $t.id('rollName').value;
		var res = notation_getter();
		if (!res.set.length || res.set.error) return;
		for (var i = 0; i < store.length; ++i) {
			if (store[i].savename == name) { store[i].roll = set.value; break; }
		}
		if (i == store.length) store.push ({ id:Math.floor(Math.random() * 900000 + 100000), savename:name, roll:set.value });
		localStorage.setItem('Bitstream630_qwEeLK_store', JSON.stringify(store));
		$t.id('saveBlock').style.display = 'none';
		refresh_store();
	});
...
var storeHtml = $t.id('savedRolls');
var store = JSON.parse (localStorage.getItem('Bitstream630_qwEeLK_store')) || [];
function refresh_store() {
	storeHtml.innerHTML = store.reduceRight(function (a, c) {
		return a + '<div class="d-flex w-100 justify-content-between list-group-item list-group-item-action px-0 py-1"><a href="javascript:rollStore(' + c.id + ')" class="list-group-item-action"><div><strong class="large">' + c.savename + '</strong><br>' + c.roll + '</div></a><div><a class="btn btn-outline-danger btn-sm small mt-2" href="javascript:remStore(' + c.id + ')">X</a></div></div>';
	}, '');
}
function remStore(id) {
	for (var i = 0; i < store.length; ++i) {
		if (store[i].id == id) {
			store.splice(i,1);
			localStorage.setItem('Bitstream630_qwEeLK_store', JSON.stringify(store));
...
function rollStore(id) {
	console.log('Rolling');
	for (var i = 0; i < store.length; ++i) {
		if (store[i].id == id) {
			$t.id('rollConfig').value = store[i].roll;
			$t.id('throw').dispatchEvent(new Event('mouseup'));
			return;
		}
	}
}
